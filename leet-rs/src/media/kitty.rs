//! Kitty graphics protocol: transmission and Unicode-placeholder placement.
//!
//! Images are transmitted once as PNG data with virtual placement (`U=1`);
//! the pane then renders a grid of `U+10EEEE` placeholder cells whose
//! foreground color encodes the image ID and whose combining diacritics
//! encode the cell's row/column. The terminal composites the image over
//! those cells, which keeps placement compatible with cell-based rendering.

/// The Kitty image placeholder code point.
pub const PLACEHOLDER: char = '\u{10EEEE}';

/// Maximum base64 payload bytes per graphics escape chunk.
const MAX_CHUNK_SIZE: usize = 4096;

/// Row/column diacritics from the Kitty spec's rowcolumn-diacritics.txt.
pub const DIACRITICS: [char; 283] = [
...
/// The diacritic encoding row/column `i`, or the first one when out of range.
pub fn diacritic(i: usize) -> char {
    DIACRITICS[if i < DIACRITICS.len() { i } else { 0 }]
}

use base64::Engine;

/// Encodes a PNG image as a chunked Kitty transmit-and-put APC sequence with
/// virtual placement at a (cols, rows) cell rectangle.
pub fn encode_transmit(png: &[u8], id: u32, cols: u16, rows: u16) -> String {
    ...
    while offset < bytes.len() || first {
        ...
        out.push_str("\x1b_G");
        if first {
            out.push_str(&format!("a=T,t=d,f=100,i={id},c={cols},r={rows},U=1,q=2"));
        } else {
            out.push_str("q=2");
        }
        if !(first && last) {
...
/// The escape sequence deleting a transmitted image (and its placements).
pub fn delete_image(id: u32) -> String {
    format!("\x1b_Ga=d,d=I,i={id},q=2\x1b\\")
}

/// Whether the terminal environment signals Kitty graphics support.
pub fn terminal_signals_kitty_graphics() -> bool {
    for var in [
        "KITTY_WINDOW_ID",
    ...
    matches!(term.as_str(), "xterm-ghostty" | "xterm-kitty")
}
