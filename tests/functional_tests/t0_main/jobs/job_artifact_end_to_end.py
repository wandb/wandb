...
cmd = ["python", "job_artifact_creation.py"]

subprocess.check_call(cmd)

api = wandb.Api()
job = api.job("job-source-test-job_artifact_creation.py:v0")
assert job._job_artifact is not None
assert job.name == "job-source-test-job_artifact_creation.py:v0"
...
kwargs = {
    "uri": None,
    "job": "job-source-test-job_artifact_creation.py:v0",
    "api": internal_api,
    "launch_spec": {},
...
if platform.system() == "Windows":
    assert to_forward_slash_path(
        lp.get_single_entry_point().compute_command({})[1]
    ) == ("job_artifact_creation.py")
