...
def test_create_kaniko_job_instance_gcp(mock_kubernetes_client, runner):
    build_config = {
        "cloud-provider": "gcp",
        "build-context-store": "gcs",
        "credentials": {
    ...
    with runner.isolated_filesystem():
        ...
        job_name = "test_job_name"
        config_map_name = "wandb-launch-build-context"
        repo_url = "us-central1-docker.pkg.dev/project/registry/repo"
        image_tag = "image_tag:12345678"
        context_path = "./test/context/path/"
        job = builder._create_kaniko_job(
            job_name,
        ...
        assert job["spec"]["template"]["spec"]["containers"][0]["volume_mounts"] == [
            {"name": "gcp-secret", "mount_path": "/secret", "read_only": True},
        ]
        assert job["spec"]["template"]["spec"]["containers"][0]["env"] == [
