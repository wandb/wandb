...
def test_repo_job_creation(test_settings, user, wandb_init):
    ...
    artifact = api.artifact(f"{user}/{run._settings.project}/{job_name}:latest")
    assert artifact.name == f"{job_name}:latest"
...
def test_artifact_job_creation(test_settings, runner, user, wandb_init):
    with runner.isolated_filesystem():
        ...
        artifact = api.artifact(f"{user}/uncategorized/{name}:latest")
        assert artifact.name == f"{name}:latest"
