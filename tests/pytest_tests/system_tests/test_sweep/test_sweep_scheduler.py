...
def test_sweep_scheduler_entity_project_sweep_id(
    ...
    monkeypatch.setattr(
        ...
        lambda _x: Mock(spec=["finish", "log", "_is_finished"]),
    )
...
def test_launch_sweep_scheduler_wandb_run(user, monkeypatch, wandb_init):
    monkeypatch.setattr(
        "wandb.sdk.launch.sweeps.scheduler.Scheduler._init_wandb_run",
        # TODO(gst): How to fix this? when wandb_init, causes "unsupported type" on settings load
        lambda _x: Mock(spec=["finish", "log", "_is_finished"]),
    )
    _project = "test-project"
    _image_uri = "some-image-wow"
