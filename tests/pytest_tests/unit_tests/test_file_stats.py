...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda stats: stats.init_file(Path("foo"), 10),
            stats.FileCountsByCategory(artifact=0, wandb=0, media=0, other=1),
        ),
        (
            lambda stats: stats.init_file(Path("foo"), 10, is_artifact_file=True),
            stats.FileCountsByCategory(artifact=1, wandb=0, media=0, other=0),
        ),
        (
            lambda stats: stats.init_file(Path("wandb", "foo"), 10),
            stats.FileCountsByCategory(artifact=0, wandb=1, media=0, other=0),
        ),
        (
            lambda stats: stats.init_file(Path("media", "foo"), 10),
            stats.FileCountsByCategory(artifact=0, wandb=0, media=1, other=0),
        ),
        (
            lambda stats: (
                stats.init_file(Path("foo"), 10),
                stats.init_file(Path("wandb/bar"), 10),
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda stats: (
                stats.init_file(Path("foo"), 10),
                stats.update_uploaded_file(Path("foo"), 7),
            ),
        ...
        (
            lambda stats: (
                stats.init_file(Path("foo"), 10),
                stats.set_file_deduped(Path("foo")),
            ),
...
def test_failed_file_resets_summary_uploaded_bytes():
    s = stats.Stats()
    s.init_file(Path("foo"), 10)
    s.init_file(Path("bar"), 10)
    s.update_uploaded_file(Path("foo"), 7)
    s.update_uploaded_file(Path("bar"), 8)

    assert s.summary().uploaded_bytes == 15

    s.update_failed_file(Path("foo"))
