...
def test_create_build_ctx_override_dockerfile(mock_launch_project, tmp_path, no_buildx):
    """Test that we set the build context to the parent directory of override Dockerfile."""
    (tmp_path / "subdir").mkdir()
    (tmp_path / "subdir/Dockerfile").write_text("FROM python:3.7")
    mock_launch_project.override_dockerfile = "subdir/Dockerfile"
    mock_launch_project.project_dir = tmp_path
    dest = Path(build._create_docker_build_ctx(mock_launch_project, "AUTOGENERATED"))
    assert (dest / "Dockerfile").read_text() == "FROM python:3.7"
