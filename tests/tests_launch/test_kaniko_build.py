...
@pytest.fixture
def mock_V1PodSpec(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1PodSpec", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1Volume(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1Volume", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1JobSpec(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1JobSpec", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1Job(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1Job", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1PodTemplateSpec(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1PodTemplateSpec", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1Container(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1Container", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1VolumeMount(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1VolumeMount", return_kwargs)
    yield return_kwargs


@pytest.fixture
def mock_V1ConfigMapVolumeSource(monkeypatch):
    monkeypatch.setattr(kubernetes.client, "V1ConfigMapVolumeSource", return_kwargs)
    yield return_kwargs
...
def test_create_kaniko_job_static(mock_kubernetes_client, runner):
    ...
    with runner.isolated_filesystem():
        ...
        assert job["spec"]["template"]["spec"]["containers"][0]["args"] == [
            "--context=./test/context/path/",
            "--dockerfile=/etc/config/Dockerfile",
            "--destination=image_tag:12345678",
            "--cache=true",
            "--cache-repo=repository-url",
        ]
        ...
        assert job["spec"]["template"]["spec"]["volumes"] == [
            {
                ...
                "config_map": {"name": "wandb-launch-build-context"},
            },
            {
                "name": "docker-config",
...
def test_build_image(monkeypatch, mock_kubernetes_client, runner):
    build_config = {
        "cloud-provider": "AWS",
        "build-context-store": "s3",
        "credentials": {
