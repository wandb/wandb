...
try:
    ...
    import paddle.nn.functional as F  # noqa: N812
except ImportError:
...
def dummy_paddle_tensor(size, requires_grad=True):
    return paddle.ones(size, requires_grad=requires_grad)


class DynamicLayer(nn.Layer):
    def __init__(self):
        super().__init__()
...
class EmbModelWrapper(nn.Layer):
...
class Discrete(nn.Layer):
    def __init__(self):
        super().__init__()

    def forward(self, x):
        return nn.functional.softmax(x, axis=0)


class DiscreteModel(nn.Layer):
...
class ParameterLayer(nn.Layer):
    def __init__(self):
        super().__init__()
        self.params = nn.ParameterList(
            [nn.Parameter(paddle.ones(10, 10)) for i in range(10)]
        )
        self.otherparam = nn.Parameter(paddle.Tensor(5))
...
class Sequence(nn.Layer):
    ...
    def forward(self, input, future=0):
        outputs = []
        h_t = dummy_paddle_tensor((input.size(0), 51))
        c_t = dummy_paddle_tensor((input.size(0), 51))
        h_t2 = dummy_paddle_tensor((input.size(0), 51))
        c_t2 = dummy_paddle_tensor((input.size(0), 51))

        for _, input_t in enumerate(input.chunk(input.size(1), axis=1)):
        ...
        for _ in range(future):  # if we should predict the future
            ...
            output = self.linear(h_t2)
            outputs += [output]
        outputs = paddle.stack(outputs, 1).squeeze(2)
        return outputs
...
def test_embedding_dict_watch(wandb_backend_spy):
    ...
    opt = paddle.optim.Adam(params=model.parameters())
    inp = paddle.randint(16, [8, 5])
    out = model(inp)
...
def test_sequence_net(user):
    ...
    with wandb.init() as run:
        ...
        output = net.forward(dummy_paddle_tensor((97, 100)))
        output.backward(paddle.zeros((97, 100)))
...
def test_multi_net(user):
    ...
    with wandb.init() as run:
        ...
        output1 = net1.forward(dummy_paddle_tensor((64, 1, 28, 28)))
        output2 = net2.forward(dummy_paddle_tensor((64, 1, 28, 28)))
        grads = paddle.ones(64, 10)
