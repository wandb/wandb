...
def test_syncs_resumed_offline_run(
    ...
    runner.invoke(
        cli.beta,
        f"sync {run1.sync_dir} {run2.sync_dir}",
    )
...
@pytest.mark.parametrize("resume", ("never", "auto", None))
def test_syncs_offline_run_without_resume__overwrites(
    ...
    `resume="auto"` belongs here rather than with the resuming modes: it
    resolves to a run ID in the client and never sets the resume intent on
    the run record.
    """
    with wandb.init(mode="offline") as run1:
        run1.log({"x": "a"})
