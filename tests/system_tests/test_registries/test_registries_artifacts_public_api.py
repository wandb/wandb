...
def api(user, team_and_org: TeamAndOrgNames) -> wandb.Api:
    return wandb.Api(overrides={"organization": team_and_org.org, "entity": team_and_org.team})
...
@pytest.fixture
def linked_artifact(source_artifact: Artifact, registry: Registry, target_collection_name: str, org_entity: str) -> str:
    return source_artifact.link(f"{org_entity}/{registry.full_name}/{target_collection_name}")
...
def test_fetch_registry_artifact(
    ...
    artifact = api.artifact(f"{org}/{registry.full_name}/{target_collection_name}:latest")
    ...
    artifact = api.artifact(f"{org_entity}/{registry.full_name}/{target_collection_name}:latest")
    assert artifact.collection is not None
    assert artifact.collection.name == target_collection_name
    assert artifact.entity == org_entity
    assert artifact.project == registry.full_name
    ...
    with pytest.raises(wandb.errors.CommError):
        artifact = api.artifact(f"invalid-entity/{registry.full_name}/{target_collection_name}:latest")
    
    artifact = api.artifact(f"{logged_artifact.qualified_name}")
    assert artifact.collection is not None
    assert artifact.collection.name == logged_artifact.collection.name
    assert artifact.entity == logged_artifact.entity
    assert artifact.project == logged_artifact.project