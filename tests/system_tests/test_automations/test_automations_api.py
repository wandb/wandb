...
import requests
import wandb
from pytest import fixture, mark, raises, xfail
...
def test_create_automation_via_api(
    ...
    assert api.automation(entity=user, name=name) == automation
    assert api.automation(name=name) == automation

    # Delete the automation for good measure
    api.delete_automation(automation)
    assert len(list(api.automations(name=name))) == 0


def test_create_automation_via_instance_method(
    user: str,
    api: wandb.Api,
    event,
    action,
    ...
    automation = (event >> action).save(name=name, description="test-description")
...
def test_create_existing_automation_raises_by_default(
    ...
    automation = api.create_automation((event >> action), name=automation_name)
...
def test_create_existing_automation_fetches_existing_if_requested(
    ...
    fetched_automation = api.automation(name=automation_name)

    assert automation1 == automation2 == fetched_automation
...
def test_create_automation_for_run_metric_event(
    ...
    api,
    automation_name: str,
    server_supported_event_types: set[str],
):
    ...
    if event.event_type.value not in server_supported_event_types:
