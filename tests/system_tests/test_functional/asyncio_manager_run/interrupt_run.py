...
if __name__ == "__main__":
    ...
    asyncer.start()
    asyncer.run(_init)
    asyncer.run_soon(_print_queue)

    try:
        asyncer.run(_add_to_queue_then_sleep)
    except KeyboardInterrupt:
        print("Got first interrupt", file=sys.stderr)
    else:
        print("FAIL: Not interrupted", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("Got second interrupt", file=sys.stderr)
