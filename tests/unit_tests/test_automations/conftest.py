...
def artifact_collection(
    ...
    """
    # ArtifactCollection.load() is called on *instantiation*, so we have to
    # patch it to avoid a live API call
    ...
    return ArtifactCollection(
        ...
        type="dataset",
        # mocked `attrs` dict expected by the constructor
        attrs=defaultdict(
            lambda: None,  # handle missing keys
            id=random_graphql_id(prefix="ArtifactCollection"),
            aliases={"edges": []},
            tags={"edges": []},
...
def project(mock_client: Mock) -> Project:
    ...
    return Project(
        ...
        attrs=defaultdict(
            lambda: None,  # handle missing keys
            id=random_graphql_id(prefix="Project"),
        ),
    )
