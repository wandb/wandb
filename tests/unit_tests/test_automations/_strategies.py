...
def ints_or_floats() -> SearchStrategy[int | float]:
    """Generates integers or floating-point numbers, e.g. 1, 1.5, 2, etc."""
...
def scalars() -> SearchStrategy[Scalar]:
...
def filter_expr_dicts() -> SearchStrategy[dict[str, Any]]:
    """Generates dicts representing valid MongoDB filter expressions on a specific field.
...
def op_dicts() -> SearchStrategy[dict[str, Any]]:
    """Generates dicts representing valid MongoDB operators.
