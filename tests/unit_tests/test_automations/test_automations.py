...
def action_type(request) -> ActionType:
    return request.param


@fixture
def scope(request, scope_type) -> public.ArtifactCollection | public.Project:
...
@fixture
def action(action_type) -> DoNotification | DoWebhook:
...
class TestDeclarativeEventSyntax:
    ...
    thresholds: SearchStrategy[float | int] = integers() | finite_floats

    @settings(suppress_health_check=[HealthCheck.differing_executors])
    @given(name=names, window=window_sizes, threshold=thresholds)
    def test_agg_metric_gt_threshold(self, name: str, window: int, threshold: float):
        metric_expr = RunEvent.metric(name).average(window)
        assert (metric_expr > threshold) == metric_expr.gt(threshold)

    @settings(suppress_health_check=[HealthCheck.differing_executors])
    @given(name=names, window=window_sizes, threshold=thresholds)
    def test_agg_metric_gte_threshold(self, name: str, window: int, threshold: float):
        metric_expr = RunEvent.metric(name).average(window)
        assert (metric_expr >= threshold) == metric_expr.gte(threshold)

    @settings(suppress_health_check=[HealthCheck.differing_executors])
    @given(name=names, window=window_sizes, threshold=thresholds)
    def test_agg_metric_lt_threshold(self, name: str, window: int, threshold: float):
        metric_expr = RunEvent.metric(name).average(window)
        assert (metric_expr < threshold) == metric_expr.lt(threshold)

    @settings(suppress_health_check=[HealthCheck.differing_executors])
    @given(name=names, window=window_sizes, threshold=thresholds)
    def test_agg_metric_lte_threshold(self, name: str, window: int, threshold: float):
        metric_expr = RunEvent.metric(name).average(window)
