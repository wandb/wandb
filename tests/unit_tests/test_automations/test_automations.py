...
def action(action_type) -> DoNotification | DoWebhook | DoLaunchJob:
    if action_type is TriggeredActionType.NOTIFICATION:
        return DoNotification(
            integration_id=make_graphql_id("Integrations"),
            title="Test title",
...
def test_define_new_automation(
    ...
    if (event_filter_json := params["eventFilter"]) is not None:
        ...
        if event_type is EventTriggeringConditionType.RUN_METRIC:
            run_filter = json.loads(prepared_filter["run_filter"])
            metric_filter = json.loads(prepared_filter["metric_filter"])
