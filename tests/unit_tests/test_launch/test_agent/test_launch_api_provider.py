...
async def test_get_api_returns_job_api(launch_api_provider, mock_agent_api):
    ...
    with patch("wandb.sdk.launch.agent.launch_api_provider.Api") as mock_api:
        mock_job_api = MagicMock()
        mock_api.return_value = mock_job_api
...
@pytest.mark.asyncio
async def test_get_api_no_job_key_returns_agent_api(
    launch_api_provider, mock_agent_api
):
...
async def test_api_caching_different_keys(launch_api_provider):
    ...
    with patch("wandb.sdk.launch.agent.launch_api_provider.Api") as mock_api:
        ...
        mock_api.side_effect = [mock_job_api1, mock_job_api2]
...
@pytest.mark.asyncio
async def test_error_handling_fallback_to_agent_api(
    launch_api_provider, mock_agent_api, exception_source
):
    ...
    if exception_source == "get_job_api_key":
        ...
        with patch("wandb.sdk.launch.agent.launch_api_provider.Api") as mock_api:
            mock_api.side_effect = Exception("API creation failed")
            api = await launch_api_provider.get_api(job_tracker)
