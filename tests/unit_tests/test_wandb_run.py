...
def _get_pinned_keys_from_records(record_q):
    """Extract pinned_keys values from config records in the queue."""
    import json
    ...
    while not record_q.empty():
        ...
        if record.HasField("config"):
            for item in record.config.update:
                ...
                if nested == ["_wandb", "pinned_keys"]:
                    results.append(json.loads(item.value_json))
    return results


class TestPinConfigKeys:
    def test_publishes_pinned_keys(self, mock_run, record_q):
        run = mock_run()
        run.pin_config_keys(["lr", "links"])
        results = _get_pinned_keys_from_records(record_q)
        assert results == [["lr", "links"]]

    def test_replaces_on_second_call(self, mock_run, record_q):
        run = mock_run()
        run.pin_config_keys(["key1", "key2"])
        run.pin_config_keys(["key3"])
        results = _get_pinned_keys_from_records(record_q)
        assert results[-1] == ["key3"]

    def test_empty_list_clears_pins(self, mock_run, record_q):
        run = mock_run()
    ...
    def test_rejects_non_sequence_input(self, mock_run):
        run = mock_run(use_magic_mock=True)
        with pytest.raises(TypeError, match="list or tuple"):
            run.pin_config_keys("not_a_list")

    def test_rejects_non_string_keys(self, mock_run):
        run = mock_run(use_magic_mock=True)
        with pytest.raises(TypeError, match="must be a string"):
            run.pin_config_keys([123])

    def test_allows_nonexistent_config_keys(self, mock_run, record_q):
        run = mock_run()
        run.pin_config_keys(["nonexistent_key"])
        results = _get_pinned_keys_from_records(record_q)
        assert results == [["nonexistent_key"]]
