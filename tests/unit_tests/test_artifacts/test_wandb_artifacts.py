...
def test_artifact_multipart_download_network_error():
    ...
    url_provider = SharedUrlProvider(
        fetch_fn=lambda: "https://invalid.com", ttl_seconds=5.0
    )
...
def test_artifact_multipart_download_disk_error():
    ...
    opener = MockOpener(file)
    url_provider = SharedUrlProvider(
        fetch_fn=lambda: "https://mocked.com", ttl_seconds=5.0
    )
    with pytest.raises(ValueError):
        with ThreadPoolExecutor(max_workers=2) as executor:
            multipart_download(
...
class TestSharedUrlProvider:
    ...
    def test_get_url_returns_fetched_url(self):
        """Test that get_url returns the URL from fetch_fn."""
        fetch_fn = Mock(return_value="https://example.com/file")
        provider = SharedUrlProvider(fetch_fn=fetch_fn, ttl_seconds=5.0)
    ...
    def test_get_url_caches_within_ttl(self):
        """Test that URLs are cached within TTL window."""
        fetch_fn = Mock(return_value="https://example.com/file")
        provider = SharedUrlProvider(fetch_fn=fetch_fn, ttl_seconds=5.0)
        ...
        assert url1 == url2
        fetch_fn.assert_called_once()  # Only fetched once

    def test_set_initial_url_sets_cached_url(self):
        """Test that set_initial_url sets the cached URL."""
        fetch_fn = Mock(return_value="https://example.com/new")
        provider = SharedUrlProvider(fetch_fn=fetch_fn, ttl_seconds=5.0)

        provider.set_initial_url("https://example.com/initial")
        url = provider.get_url()

        assert url == "https://example.com/initial"
        fetch_fn.assert_not_called()  # Should not fetch since initial was set
    ...
    def test_invalidate_forces_refresh(self):
        ...
        def fetch_fn():
            nonlocal call_count
            call_count += 1
            return f"https://example.com/url{call_count}"

        provider = SharedUrlProvider(fetch_fn=fetch_fn, ttl_seconds=5.0)

        url1 = provider.get_url()
        assert url1 == "https://example.com/url1"

        provider.invalidate()
        url2 = provider.get_url()

        assert url2 == "https://example.com/url2"
        assert call_count == 2
...
def test_artifact_multipart_download_max_retries_exceeded():
    ...
    url_provider = SharedUrlProvider(
        fetch_fn=lambda: "https://example.com/new-url", ttl_seconds=0.01
    )
