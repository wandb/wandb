...
class TestUpload:
    ...
    class TestStats:
        @pytest.mark.parametrize(
            ...
            [
                lambda _: None,
                lambda size: AsyncMock(side_effect=lambda progress: progress(size, size)),
            ],
        )
        ...
        @pytest.mark.parametrize(
            ...
            [
                None,
                AsyncMock(side_effect=Exception("save_fn failed")),
            ],
        )
        ...
        def test_update_on_deduped(
            ...
            q = queue.Queue()
            cmd = make_request_upload(f, save_fn=AsyncMock(return_value=deduped))
            q.put(cmd)
