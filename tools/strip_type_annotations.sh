...
for dir in $CODEMOD_DIRS; do
  ...
  dest="$CODEMOD_SRC_BASE/${dir}${CODEMOD_DEST_SUFFIX}/"
  dest_tmp="$CODEMOD_SRC_BASE/${dir}${CODEMOD_DEST_SUFFIX}${CODEMOD_DEST_SUFFIX_TMP}/"
  if $check; then
    dest_orig=$dest
    dest=$dest_tmp
  fi
  ...
  if $check; then
    ...
    set +e
    diff --exclude="*.pyc" --exclude="__pycache__" -q $dest_orig $dest_tmp
    result="$?"
    set -e
    rm -f ${dest}*
    rmdir $dest
