...
from ariadne_codegen.client_generators.constants import ANNOTATED
...
class PydanticModelRewriter(ast.NodeTransformer):
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> ast.AnnAssign:
        ...
        if is_pydantic_field(node.value):
            ...
            node.value = field_kws.pop("default", None)
        ...
        if field_kws:
            # Convert the remaining `Field(...)` keyword args back to an `Annotated[...]` type hint
            node.annotation = generate_subscript(
                ast.Name(ANNOTATED),
                ast.Tuple([node.annotation, generate_pydantic_field(field_kws)]),
            )
