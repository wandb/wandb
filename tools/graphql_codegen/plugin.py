...
class GraphQLCodegenPlugin(Plugin):
    ...
    def generate_input_field(
        ...
        return self._apply_field_constraints(field_implementation, input_field)
    ...
    def generate_result_field(
        ...
        if gql_field := self._get_field_def(field, operation_definition):
            return self._apply_field_constraints(field_implementation, gql_field)
    ...
    def _get_field_def(
        ...
        # NOTE:
        # - The `field` node here is parsed from inside the GraphQL operation (query/mutation/fragment/etc.)
        # - However, it doesn't know what original, typed field _definition_ it represents in the schema.
        #
    ...
    def _apply_field_constraints(
        ...
        """
        if not (field_kws := self._parse_constraints(gql_field)):
            return ann
