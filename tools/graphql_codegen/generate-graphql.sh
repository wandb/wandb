...
elif [ ! -f "$SCHEMA_SYMLINK_PATH" ]; then
    echo "[INFO] Schema symlink does not exist: $SCHEMA_SYMLINK_PATH"
    ln -svf "$SCHEMA_FILE_PATH" "$SCHEMA_SYMLINK_PATH"
else
    echo "[INFO] Schema file already exists, skipping download: $SCHEMA_FILE_PATH"
    echo "[INFO] Schema symlink already exists, skipping symlink: $SCHEMA_SYMLINK_PATH"
fi
