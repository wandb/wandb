...
func TestInvalidHeader(t *testing.T) {
	header := runstream.HeaderOptions{
...
func TestOpenCreateStore(t *testing.T) {
	...
	store := runstream.NewStore(tmpFile.Name())
...
func TestOpenReadStore(t *testing.T) {
	...
	store2 := runstream.NewStore(tmpFile.Name())
...
func TestStoreHeader_Write_Error(t *testing.T) {
	store := runstream.NewStore("non_existent_dir/file")
	err := store.Open(os.O_WRONLY)
	assert.Error(t, err)
}
