...
import (
	...
	ft "github.com/wandb/wandb/core/internal/filetransfer"
...
type ArtifactSaver struct {
	...
	FileTransferManager *ft.Manager
	// Input.
	Artifact    *pb.ArtifactRecord
	HistoryStep int64
	StagingDir  string
}
...
func (as *ArtifactSaver) createArtifact() (
	...
	response, err := gql.CreateArtifact(
		...
		&as.Artifact.RunId,
		as.Artifact.Digest,
		corelib.NilIfZero(as.Artifact.Description),
		aliases,
		corelib.NilIfZero(as.Artifact.Metadata),
		corelib.NilIfZero(as.Artifact.TtlDurationSeconds),
		corelib.NilIfZero(as.HistoryStep),
		corelib.NilIfZero(as.Artifact.DistributedId),
...
func (as *ArtifactSaver) uploadFiles(artifactID string, manifest *Manifest, manifestID string, outChan chan<- *pb.Record) error {
	...
	for numDone < len(fileSpecs) {
		...
		if len(fileSpecsBatch) > 0 {
			...
			for i, edge := range response.CreateArtifactFiles.Files.Edges {
				...
				task.AddCompletionCallback(func(task *ft.Task) {
...
func (as *ArtifactSaver) resolveClientIDReferences(manifest *Manifest) error {
	...
	for name, entry := range manifest.Contents {
		if entry.Ref != nil && strings.HasPrefix(*entry.Ref, "wandb-client-artifact:") {
			...
			serverIdHex, err := hashlib.B64ToHex(serverId)
			if err != nil {
				return err
			}
...
func (as *ArtifactSaver) uploadManifest(manifestFile string, uploadUrl *string, uploadHeaders []string, outChan chan<- *pb.Record) error {
	resultChan := make(chan *ft.Task)
	task := &ft.Task{
		Type:    ft.UploadTask,
		Path:    manifestFile,
		Url:     *uploadUrl,
					...
					Request: &pb.Request{
						RequestType: &pb.Request_FileTransferInfo{
							FileTransferInfo: &pb.FileTransferInfoRequest{
								...
								Size:       task.Size,
								Processed:  task.Size,
								FileCounts: &pb.FileCounts{ArtifactCount: 1},
							},
						},
					},
				},
...
func (as *ArtifactSaver) Save(ch chan<- *pb.Record) (artifactID string, rerr error) {
