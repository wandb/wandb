		...
		for hasNextPage {
			...
			if len(manifestEntriesBatch) > 0 {
				for _, entry := range manifestEntriesBatch {
					...
					exists, err := fileslib.FileExists(downloadLocalPath)
					if err != nil {
						return err
					}
					if exists {
						existingDigest, err := hashlib.ComputeFileB64MD5(downloadLocalPath)
						if err != nil {
							return err
						}
