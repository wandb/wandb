...
func NewManifestFromProto(proto *pb.ArtifactManifest) (Manifest, error) {
	manifest := Manifest{
	...
	for _, entry := range proto.Contents {
		...
		manifest.Contents[entry.Path] = ManifestEntry{
			Digest:          entry.Digest,
			BirthArtifactID: corelib.NilIfZero(entry.BirthArtifactId),
			Ref:             corelib.NilIfZero(entry.Ref),
			Size:            entry.Size,
			Extra:           extra,
			LocalPath:       corelib.NilIfZero(entry.LocalPath),
		}
	}
	return manifest, nil
}
...
func (m *Manifest) WriteToFile() (filename string, digest string, rerr error) {
	return WriteJsonToFileWithDigest(m)
}
