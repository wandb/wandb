...
func TestDefaultRetryPolicy(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			retry, err := retryableclient.DefaultRetryPolicy(context.Background(), resp, nil)
...
func TestUpsertBucketRetryPolicy(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			retry, err := retryableclient.UpsertBucketRetryPolicy(context.Background(), resp, nil)
...
func TestCheckRetry(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), retryableclient.CtxRetryPolicyKey, mockRetryPolicy)
	resp := httptest.NewRecorder().Result()

	resp.StatusCode = http.StatusForbidden
	retry, err := retryableclient.CheckRetry(ctx, resp, nil)
	assert.True(t, retry)
	assert.Nil(t, err)
