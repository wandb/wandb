...
func TestBase(t *testing.T) {
	testCases := []struct {
		...
		{url: "gs://my-bucket/log/dir", expected: "dir"},
		{url: "gs://my-bucket/log/dir/", expected: "dir"},
		{url: "gs://my-bucket/", expected: ""},
		{url: "local/path", expected: "path"},
		{url: "/", expected: ""},

		// NOTE: The following test would only pass on Windows because
		//   filepath uses a different implementation on Unix systems that
		//   ignores volume names.
		// {url: "C:\\", expected: ""},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.url, func(t *testing.T) {
			...
			assert.Equal(t, testCase.expected, path.Base())
		})
	}
}
