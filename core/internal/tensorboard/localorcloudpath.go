...
func ParseTBPath(pathURL string) (*LocalOrCloudPath, error) {
	isS3 := strings.HasPrefix(pathURL, "s3://")
	isGS := strings.HasPrefix(pathURL, "gs://")
	isAZ := strings.HasPrefix(pathURL, "az://")
	...
	if isS3 || isGS || isAZ {
		...
		trimmedURLPath := strings.Trim(url.EscapedPath(), "/")
		...
		case isAZ:
			...
			if len(urlPathParts) < 2 {
				return nil, fmt.Errorf("invalid Azure cloud URL: %q", pathURL)
			}

			accountName := url.Host
			bucketName := urlPathParts[0]

			path = &CloudPath{
				Scheme:     "azblob",
				BucketName: fmt.Sprintf("%s/%s", accountName, bucketName),
				Path:       strings.Join(urlPathParts[1:], "/"),
			}

		default:
			return nil, fmt.Errorf("invalid cloud URL: %q", pathURL)
		}

		return &LocalOrCloudPath{CloudPath: path}, nil
	}

	path, err := paths.Absolute(pathURL)
	if err != nil {
...
//
// If the path is empty or doesn't have a base name (such as the root path "/"),
// Base returns "".
func (p *LocalOrCloudPath) Base() string {
	switch {
	case p.CloudPath != nil:
		parts := strings.Split(p.CloudPath.Path, "/")
	...
	case p.LocalPath != nil:
		// The bases of "C:\" and "C:/" are "\" and "/".
		return strings.Trim(filepath.Base(string(*p.LocalPath)), "\\/")
