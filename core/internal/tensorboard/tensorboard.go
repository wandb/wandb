...
func (tb *TBHandler) Handle(record *service.TBRecord) {
	...
	go func() {
		defer tb.wg.Done()
		tb.convertToRunHistory(stream.Events())
	}()
...
func (tb *TBHandler) convertToRunHistory(events <-chan *tbproto.TFEvent) {
	accum := HistoryAccumulator{}
	for event := range events {
		tb.logger.Debug("tensorboard: processed event", "event", event)

		if history := accum.Add(event); history != nil {
