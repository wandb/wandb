...
func (s *Sender) sendPreempting(record *spb.RunPreemptingRecord) {
	if s.fileStream == nil || s.errorAfterExit("sendPreempting") {
		return
	}
...
func (s *Sender) sendHistory(record *spb.HistoryRecord) {
	if s.fileStream == nil || s.errorAfterExit("sendHistory") {
		return
	}
...
func (s *Sender) sendSummary(_ *spb.Record, summary *spb.SummaryRecord) {
	if s.errorAfterExit("sendSummary") {
		return
	}
...
func (s *Sender) sendSystemMetrics(record *spb.StatsRecord) {
	if s.fileStream == nil || s.errorAfterExit("sendSystemMetrics") {
		return
	}
...
func (s *Sender) sendOutput(_ *spb.Record, _ *spb.OutputRecord) {
	if s.errorAfterExit("sendOutput") {
		return
	}
...
func (s *Sender) sendOutputRaw(_ *spb.Record, outputRaw *spb.OutputRawRecord) {
	if s.errorAfterExit("sendOutputRaw") {
		return
	}
...
// errorAfterExit logs an error and returns true if exitRecord isn't nil.
//
// It is used to avoid panics when unexpectedly receiving certain run-modifying
// records after an Exit, which is invalid.
func (s *Sender) errorAfterExit(method string) bool {
	if s.exitRecord == nil {
		return false
	}

	s.logger.CaptureError(fmt.Errorf("sender: %s after exit", method))
	return true
}
