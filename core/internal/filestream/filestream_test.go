...
func NewFilestreamTest(tName string, fn func(fs *filestream.FileStream)) *filestreamTest {
	...
	fs := filestream.NewFileStream(
		...
		filestream.WithHttpClient(retryableclient.NewRetryClient(
			retryableclient.WithRetryClientHttpAuthTransport(tserver.settings.GetApiKey().GetValue()),
			retryableclient.WithRetryClientLogger(tserver.logger))),
	)
	fs.Start()
...
func NewHistoryRecord() *pb.Record {
	msg := &pb.Record{
		RecordType: &pb.Record_History{
			History: &pb.HistoryRecord{
				Step: &pb.HistoryStep{Num: 0},
				Item: []*pb.HistoryItem{
