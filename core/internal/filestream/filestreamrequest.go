...
func (b *FileStreamRequestBuffer) Get() (
	...
	*fileStreamBufferAdvancer,
	...
	if len(b.HistoryLines) > 0 {
		data := FileStreamFileData{
			Offset:  b.HistoryLineNum,
			Content: make([]string, 0, len(b.HistoryLines)),
		}

		// Limit the amount of data we upload per request.
		totalBytes := len(b.HistoryLines[0])
		data.Content = append(data.Content, b.HistoryLines[0])
		advancer.uploadedHistoryLines = 1

		for _, line := range b.HistoryLines[1:] {
			if totalBytes+len(line) > maxRequestHistoryBytes {
				break
			}
			totalBytes += len(line)

			data.Content = append(data.Content, line)
			advancer.uploadedHistoryLines += 1
		}

		if advancer.uploadedHistoryLines < len(b.HistoryLines) {
			hasMoreDataAfter = true
		}

		req.Files[HistoryFileName] = data
	}
...
func (b *fileStreamBufferAdvancer) Advance() *FileStreamRequestBuffer {
	...
	return &FileStreamRequestBuffer{
		...
		HistoryLines:   b.prev.HistoryLines[b.uploadedHistoryLines:],
		HistoryLineNum: b.prev.HistoryLineNum + b.uploadedHistoryLines,
