...
func TestGet_HistoryLimit(t *testing.T) {
	bigLine := strings.Repeat("b", 10<<19)
	hugeLine := strings.Repeat("a", 10<<20)
	buf := &FileStreamRequestBuffer{
		HistoryLines: []string{
			// Should take up one request.
			hugeLine,

			// Next two lines should be uploaded together.
			bigLine,
			"small line",

			// Next two lines should go together as well.
			bigLine,
			bigLine,
		},
	}
	...
	req, isDone, advance := buf.Get()
	assert.False(t, isDone)
	assert.Equal(t, []string{hugeLine}, req.Files[HistoryFileName].Content)

	buf = advance.Advance()
	req, isDone, advance = buf.Get()
	assert.False(t, isDone)
	assert.Equal(t,
		[]string{bigLine, "small line"},
		req.Files[HistoryFileName].Content)
