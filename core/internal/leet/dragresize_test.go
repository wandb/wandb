...
func TestBoundaryAtOverviewSeparators(t *testing.T) {
	...
	_, ok := boundaryAt(5, sb.separators[0].row+1, layout, targets)
	require.False(t, ok)

	// The same row outside the sidebar is not an overview drag.
	drag, ok := boundaryAt(100, sb.separators[0].row, layout, targets)
	...
	_, ok = boundaryAt(5, sb.separators[0].row, layout, targets)
	require.False(t, ok)
}
