...
func (b *LocalWorkspaceBackend) DisplayLabel() string {
	return "wandb dir: " + b.wandbDir
}

// localWandbDir returns the wandb directory of a local workspace, or "" for
// a remote one.
func localWandbDir(b WorkspaceBackend) string {
	if lb, ok := b.(*LocalWorkspaceBackend); ok {
		return lb.wandbDir
	}
	return ""
}
