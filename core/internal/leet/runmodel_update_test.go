...
func TestProcessRecordMsg_Run_Summary_System_FileComplete(t *testing.T) {
	...
	var m tea.Model = leet.NewRunModel("dummy", cfg, logger)
	m, _ = m.Update(tea.WindowSizeMsg{Width: 140, Height: 50})

	model := m.(*leet.RunModel)
...
func TestHeartbeat_LiveRun(t *testing.T) {
	...
	// Create model
	m := leet.NewRunModel(path, cfg, logger)
	...
	concreteModel := model.(*leet.RunModel)
...
func TestModel_HandleMouseMsg(t *testing.T) {
	...
	setupModel := func(t *testing.T) *leet.RunModel {
		logger := observability.NewNoOpLogger()
		cfg := leet.NewConfigManager(filepath.Join(t.TempDir(), "config.json"), logger)
		...
		m := leet.NewRunModel("dummy", cfg, logger)
		...
		_ = model.(*leet.RunModel).View()

		return model.(*leet.RunModel)
	}

	tests := []struct {
		name   string
		setup  func(*leet.RunModel)
		events []tea.MouseMsg
		verify func(*testing.T, *leet.RunModel)
	}{
		{
			name: "click_in_left_sidebar_clears_all_focus",
			setup: func(m *leet.RunModel) {
				m.TestSetMainChartFocus(0, 0)
			},
			events: []tea.MouseMsg{
				{X: 10, Y: 10, Button: tea.MouseButtonLeft, Action: tea.MouseActionPress},
			},
			verify: func(t *testing.T, m *leet.RunModel) {
