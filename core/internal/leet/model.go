...
// ResolveRunFile returns the .wandb file to open: the explicit runFile
// when non-empty, otherwise the latest run in wandbDir.
func ResolveRunFile(runFile, wandbDir string) (string, error) {
	if runFile != "" {
		return runFile, nil
	}
