...
func makeStatsRecord(stats map[string]float64, timeStamp *timestamppb.Timestamp) *pb.Record {
	record := &pb.Record{
		RecordType: &pb.Record_Stats{
			Stats: &pb.StatsRecord{
				StatsType: pb.StatsRecord_SYSTEM,
				Timestamp: timeStamp,
			},
		},
	...
	for k, v := range stats {
		...
		record.GetStats().Item = append(record.GetStats().Item, &pb.StatsItem{
			Key:       k,
...
func (sm *SystemMonitor) Probe() *pb.MetadataRequest {
	if sm == nil {
		return nil
	}
	systemInfo := pb.MetadataRequest{}
	for _, asset := range sm.assets {
