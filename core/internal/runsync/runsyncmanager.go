...
type RunSyncManager struct {
	...
	nextID         int
	preppedSyncOps map[string]*RunSyncOperation
...
func NewRunSyncManager() *RunSyncManager {
	return &RunSyncManager{
		preppedSyncOps:          make(map[string]*RunSyncOperation),
...
func (m *RunSyncManager) InitSync(
	...
	m.nextID++
	m.preppedSyncOps[id] = m.runSyncOperationFactory.New(
		request.Path,
...
func (m *RunSyncManager) DoSync(
	...
	m.mu.Lock()
	op, exists := m.preppedSyncOps[request.Id]
	if exists {
		m.ongoingSyncOps[request.Id] = op
		delete(m.preppedSyncOps, request.Id)
	}
	m.mu.Unlock()
