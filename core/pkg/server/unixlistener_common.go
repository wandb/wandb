...
// characters.
func listenInTempDir(namePattern string) (net.Listener, PortInfo, error) {
	sockDir, err := makeUniqueDir(os.TempDir(), namePattern)

	if err != nil {
		return nil, PortInfo{}, fmt.Errorf(
			"server: failed to make tempdir for Unix socket: %v", err)
	}
...
// listen attempts to listen on a Unix socket with the given path.
func listen(path string) (net.Listener, PortInfo, error) {
	listener, err := net.Listen("unix", path)

	if err != nil {
		return nil, PortInfo{}, fmt.Errorf(
			"server: failed to open Unix socket on %q: %v",
			path, err)
	}

	return listener, PortInfo{UnixPath: path}, nil
}
