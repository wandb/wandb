...
func NewHandler(
	...
	h := &Handler{
		ctx:    ctx,
		logger: observability.NewCoreLogger(slog.New(logger.Handler().WithGroup("handler"))),
	}
...
func (h *Handler) Do(inChan <-chan *service.Record) {
	...
	h.logger.Info("started handler")
	for record := range inChan {
		h.logger.Debug("recieved record", "type", record.RecordType)
...
func (h *Handler) handleRecord(record *service.Record) {
	...
	case nil:
		...
		h.logger.Error("error handling record", "error", err)
		panic(err)
...
func (h *Handler) handleDefer(record *service.Record, request *service.DeferRequest) {
	...
	default:
		...
		h.logger.Error("error handling defer", "error", err)
...
func (h *Handler) handleRunStart(record *service.Record, request *service.RunStartRequest) {
	...
	if h.runRecord, ok = proto.Clone(run).(*service.RunRecord); !ok {
		...
		h.logger.Error("error handling run start", "error", err)
		panic(err)
...
func (h *Handler) handleCodeSave() {
	...
	if programRelative == "" {
		h.logger.Warn("code save: program relative path is empty")
		return
	}

	programAbsolute := h.settings.GetProgramAbspath().GetValue()
	if _, err := os.Stat(programAbsolute); err != nil {
		h.logger.Warn("code save: program path does not exist", "path", programAbsolute)
...
func (h *Handler) handleMetadata(request *service.MetadataRequest) {
	...
	if err != nil {
		h.logger.Error("error marshalling metadata", "error", err)
	...
	if err := os.WriteFile(filePath, jsonBytes, 0644); err != nil {
		...
		h.logger.Error("error writing metadata file", "error", err)
		return
	}
...
func (h *Handler) writeAndSendSummaryFile() {
	...
	if err := os.WriteFile(summaryFile, []byte(jsonBytes), 0644); err != nil {
		h.logger.Error("failed to write summary file", "error", err)
	}
...
func (h *Handler) handleTBrecord(record *service.Record) {
	...
	if err != nil {
		h.logger.Error("error handling tbrecord", "error", err)
