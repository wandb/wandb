...
// TestConnection_SweepScheduler covers how a Connection routes the
// scheduler protocol's requests: both share the same
// init-then-read-response flow over a fresh pipe.
func TestConnection_SweepScheduler(t *testing.T) {
	t.Run("routes a task through its lifecycle", func(t *testing.T) {
	...
	t.Run("routes an init error from the factory", func(t *testing.T) {
		clientConn, unwound := newSchedulerTestConn(t, func(
			schedCtx context.Context,
			reqCtx context.Context,
			req *spb.SweepSchedulerClientInitRequest,
