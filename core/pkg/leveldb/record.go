...
// Next returns NextWithOffset without the offset.
func (r *Reader) Next() (io.Reader, error) {
	reader, _, err := r.NextWithOffset()
	return reader, err
}

// NextWithOffset returns a reader for the next record.
//
...
func (r *Reader) NextWithOffset() (io.Reader, int64, error) {
	startOffset := r.blockOffset + int64(r.nextChunkStart)

	r.seq++
	if r.err != nil {
		return nil, startOffset, r.err
	}
	...
	return singleReader{r, r.seq}, startOffset, nil
}
