...
- `wandb leet inspect --json` prints a run's records, one JSON object per line, or its `--summary` as one JSON object, and `--follow` keeps printing records as a running run writes them until it exits (@dmitryduev in https://github.com/wandb/wandb/pull/PRNUM)
