...
- Fixed a memory leak where every `wandb.Api()` object permanently retained a few MiB in the background service process after it was garbage collected, which could OOM long-running jobs that create one per poll (@dmitryduev in https://github.com/wandb/wandb/pull/12920)
