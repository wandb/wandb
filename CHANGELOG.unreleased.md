...
- `wandb[sandbox]` now caps the `cwsandbox` client at 0.26.x. Users can override the pin (e.g. `pip install "cwsandbox[cli]==<version>"`) in their own project to use a newer upstream client, but that combination is not verified by W&B (@nicholaspun-wandb in https://github.com/wandb/wandb/pull/12026)
