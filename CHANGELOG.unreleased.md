...
- Registry search: `collections(order=...).versions()` now returns artifact versions in collection order. Previously, chaining `.versions()` after an ordered `.collections()` call ignored the collection `order` and used a single cross-collection query (@ibindlish in https://github.com/wandb/wandb/pull/TODO)
