...
- System metrics scraped through `x_stats_dcgm_exporter` now report the DCGM profiling ratios (`smActive`, `smOccupancy`, `pipeTensorActive` and the other `DCGM_FI_PROF_*` fields) as percentages, matching the values the `wandb-xpu` sidecar reports under the same keys (@dmitryduev in https://github.com/wandb/wandb/pull/13008)
- The in-memory system metrics buffer behind `x_stats_buffer_size` no longer drops samples whose value is a whole number (@dmitryduev in https://github.com/wandb/wandb/pull/13008)
