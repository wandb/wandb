...
- `File.download()` now displays a download progress bar; it and run-metadata downloads run on wandb-core's file transfer worker pool, so a download no longer holds up other API requests (@dmitryduev in https://github.com/wandb/wandb/pull/12064)

### Fixed

- `File.download()` no longer fails after a hardcoded 5-second timeout; downloads respect the file transfer settings (@dmitryduev in https://github.com/wandb/wandb/pull/12039)
