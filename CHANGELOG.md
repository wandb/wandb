...
- Support for RFC 7523 bearer token exchange. This enhancement provides an alternative to the API key-based authentication
  method with a more secure and flexible token-based authentication mode. You configure this by setting the file path to
  the identity token (JWT) in the `WANDB_IDENTITY_TOKEN_FILE` environment variable.
