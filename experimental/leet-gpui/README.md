...
The layout is leet's workspace: a runs sidebar on the left, a central column with the metrics grid over a lower tier of system metrics and console logs, the run overview on the right, and a status bar. `1`, `2`, `4`, `[`, `]` toggle panes, `tab` cycles focus, `n`/`N` page a grid or list, and with the runs list focused `space` selects runs to chart and `p` pins the run the overview, system, and console panes follow. Lists page rather than scroll, as in leet: a list shows the page holding its cursor, moving past the edge turns the page, left and right (`a`/`d`) and the wheel turn pages, and the header says which page you are on. In a grid, moving past an edge in any direction turns the page too. The overview's environment, config, and summary are sections that page on their own, with draggable borders between them, so a large config never hides the summary. Clicking any pane or chart focuses it. Pinning is explicit and lasts the session: `p` pins, and `space` on a pinned run unpins and deselects it. Pane borders drag; `0` resets them. Panes slide in and out over 150 ms on leet's ease-out cubic curve. `c` or `r` followed by a digit sets the focused grid's columns or rows. Grids show fewer rows when their pane is short rather than charts without axes. Cells keep the size the full grid gives them so charts do not move between pages, trailing slots on a short page stay blank, a grid whose charts fit on one page uses only the rows it needs, and an empty grid says why (nothing selected, still loading, no system metrics logged, a filter matching nothing) in place of empty boxes.

Hovering a chart snaps the crosshair to the nearest logged step and shows every series' value there, never an interpolated one; a series whose nearest point is at another step says so. The crosshair is linked across charts of the same axis so the same step can be read off all of them; `i` unlinks it. The mouse wheel zooms the x axis of the chart under it around the pointer, the y axis follows the visible window, and `z` resets the focused chart.
