...
def parse_log(log_path):
    ...
    steps_per_run = int(re.search(r"# of steps in each run: (\d+)", log_content).group(1))
    metrics_per_step = int(re.search(r"# of metrics in each step: (\d+)", log_content).group(1))
    
    # Extract timing information for the run
    init_time = float(re.search(r"init_wandb\(\) time: ([\d.]+)", log_content).group(1))
    log_metrics_time = float(re.search(r"log_metrics\(\) time: ([\d.]+)", log_content).group(1))
    finish_time = float(re.search(r"finish_wandb\(\) time: ([\d.]+)", log_content).group(1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--folder", type=str, help="Test result ROOT folder (required)", required=True)
    parser.add_argument("-n", "--run_name", type=str, help="Name of this test run", required=True)
