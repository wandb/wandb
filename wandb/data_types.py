...
class Table(Media):
    ...
    def bind_to_run(self, *args, **kwargs):
        ...
        tmp_path = os.path.join(
            _get_media_tmp_dir().name, runid.generate_id() + ".table.json"
        )
...
class Audio(BatchableMedia):
    ...
    def __init__(self, data_or_path, sample_rate=None, caption=None):
        ...
        if isinstance(data_or_path, str):
            ...
            tmp_path = os.path.join(
                _get_media_tmp_dir().name, runid.generate_id() + ".wav"
            )
...
class Bokeh(Media):
    ...
    def __init__(self, data_or_path):
        ...
        if isinstance(data_or_path, str) and os.path.exists(data_or_path):
            ...
            tmp_path = os.path.join(
                _get_media_tmp_dir().name, runid.generate_id() + ".bokeh.json"
            )
            with codecs.open(tmp_path, "w", encoding="utf-8") as fp:
...
class Graph(Media):
    ...
    def bind_to_run(self, *args, **kwargs):
        ...
        tmp_path = os.path.join(
            _get_media_tmp_dir().name, runid.generate_id() + ".graph.json"
        )
        data = _numpy_arrays_to_lists(data)
        with codecs.open(tmp_path, "w", encoding="utf-8") as fp:
            util.json_dump_safer(data, fp)
