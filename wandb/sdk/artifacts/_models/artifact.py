...
class ArtifactData(ArtifactsBase):
    ...
    # # NOTE: definitely don't want these in the transport-free data model
    ...
    source_names: Frozen[FullArtifactPath]
    ...
    manifest: ArtifactManifest | DeferredArtifactManifest | None = Field(
        default_factory=lambda: ArtifactManifestV1(storage_policy=make_storage_policy())
    )
    ...
    @classmethod
    def from_artifact_fragment(cls, artifact: ArtifactFragment) -> Self:
        ...
        """
        if not (
            (source_collection := artifact.artifact_sequence)
            and source_collection.project
        ):
        ...
        return cls(
            id=artifact.id,
            entity=source_collection.project.entity_name,
            project=source_collection.project.name,
            name=source_collection.name,
            version=artifact.version_index,
            source_entity=source_collection.project.entity_name,
            source_project=source_collection.project.name,
    ...
    @classmethod
    def from_membership_fragment(cls, membership: ArtifactMembershipFragment) -> Self:
        ...
        return cls(
            id=artifact.id,
            entity=collection.project.entity_name,
