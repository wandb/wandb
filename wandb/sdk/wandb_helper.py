        ...
        for k, v in list(d.items()):
            if type(v) == dict:
                ...
                if not isinstance(k, str):
                    raise ConfigError(f"Config keys must be strings, found {k} of type {type(k)}")
                for subkey, subval in v.items():
                    if not isinstance(subkey, str):
                        raise ConfigError(f"Config keys must be strings, found {subkey} of type {type(subkey)}")
            ...
            if delimiter in k:
                ...
                for i, subkey in enumerate(subkeys):
                    ...
                    if subkey in d and not isinstance(subdict[subkey], dict):
                        ...
                        raise ConfigError(f"While un-nesting config, found key {subkey} which conflics with key {conflict_key}")
                    else:
