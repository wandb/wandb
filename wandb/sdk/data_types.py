...
_use_type_checks: bool = sys.version_info.major == 3 and sys.version_info.minor >= 9
if _use_type_checks:
    import typing as t
...
class WBValue(object):
    ...
    @classmethod
    def from_json(
        cls, json_obj: _TypeWBValueJSONAll, source_artifact: "DownloadedArtifact"
    ) -> "WBValue":
    ...
    @classmethod
    def with_suffix(cls, name: str, filetype="json") -> str:
    ...
    @staticmethod
    def init_from_json(
        json_obj: _TypeWBValueJSON,
        source_artifact: "DownloadedArtifact",
        name: str = None,
    ) -> t.Optional["WBValue"]:
...
class Media(WBValue):
    ...
    @staticmethod
    def captions(media_items: t.List[t.Type["Media"]]) -> t.Optional[t.List[str]]:
...
class Table(Media):
    ...
    def _to_table_json(
        self, max_rows: t.Optional[int] = None
...
class BoundingBoxes2D(JSONMetadata):
    ...
    def __init__(self, val: _TypeBoundingBoxes2DVal, key: str):
        """
        Args:
    ...
    def to_json(
        self, run_or_artifact: t.Union["wandb_run.Run", "DraftArtifact"]
    ) -> t.Union[_TypeBoundingBoxes2DBoxData, _TypeMediaJSONRun]:
        wandb_run, wandb_artifacts = _safe_sdk_import()
...
class Image(BatchableMedia):
    ...
    def __init__(
        ...
        if isinstance(data_or_path, Image):
            self._init_from_image(data_or_path)
        elif isinstance(data_or_path, six.string_types):
            self._init_from_path(data_or_path)
        else:
            self._init_from_data(data_or_path, mode)

        self._init_metadata(caption, grouping, classes, boxes, masks)

    def _init_from_image(self, image):
        self._grouping = image._grouping
        ...
        self._sha256 = image._sha256
        self._size = image._size
        self.format = image.format
        self.artifact_source = image.artifact_source

        # We do not want to implicitly copy boxes or masks, just the image-related data.
        # self._boxes = image._boxes
        # self._masks = image._masks
    ...
    def _init_from_path(self, path):
        ...
        self._image = pil_module.open(path)
        self._image.load()
        ext = os.path.splitext(path)[1][1:]
        self.format = ext

    def _init_from_data(self, data, mode=None):
        pil_module = util.get_module(
            "PIL.Image",
            required='wandb.Image needs the PIL package. To get it, run "pip install pillow".',
        )
        ...
        if util.is_matplotlib_typename(util.get_full_typename(data)):
            ...
            self._image = pil_module.open(buf)
        elif isinstance(data, pil_module.Image):
            self._image = data
            ...
            self._image = pil_module.fromarray(
    ...
    def _init_metadata(
        self, caption=None, grouping=None, classes=None, boxes=None, masks=None
    ):
...
class Graph(Media):
    ...
    def from_keras(cls, model):
        ...
        graph = cls()
        relevant_nodes = cls._get_relevant_nodes(model)
        layers = model.layers
