...
class Api:
    ...
    def _resolve_client_ids(
        ...
        """Resolve multiple client IDs to their corresponding server IDs.

        Args:
            client_ids: List of client IDs to resolve.

        Returns:
            A dictionary mapping client IDs to their server IDs. If a client ID
            cannot be resolved, its value will be None.
        """
        # Initialize the result dictionary
        result = {}
        
        # Check which client IDs are already in the mapping
        unresolved_ids = []
        for client_id in client_ids:
            if client_id in self._client_id_mapping:
                result[client_id] = self._client_id_mapping[client_id]
            else:
                unresolved_ids.append(client_id)
                
        # If all IDs were in the mapping, return early
        if not unresolved_ids:
            return result
            
        # Construct a query to resolve multiple client IDs in one request
        query = gql(
            """
            query ClientIDsMapping($clientIDs: [ID!]!) {
                clientIDsMapping(clientIDs: $clientIDs) {
                    clientID
                    serverID
                }
            }
        ...
        try:
            # Try to resolve multiple IDs at once
            response = self.gql(
                query,
                variable_values={
                    "clientIDs": unresolved_ids,
                },
            )
            
            if response is not None and "clientIDsMapping" in response:
                for mapping in response["clientIDsMapping"]:
                    client_id = mapping.get("clientID")
                    server_id = mapping.get("serverID")
                    if client_id is not None and server_id is not None:
                        result[client_id] = server_id
                        self._client_id_mapping[client_id] = server_id
                        
                # Add any unresolved IDs with None value
                for client_id in unresolved_ids:
                    if client_id not in result:
                        result[client_id] = None
            else:
                # Fallback: resolve IDs individually 
                for client_id in unresolved_ids:
                    result[client_id] = self._resolve_client_id(client_id)
        except Exception as e:
            # Fallback to resolving one by one if batch query fails
            print("ERROR", e)
            for client_id in unresolved_ids:
                result[client_id] = self._resolve_client_id(client_id)
