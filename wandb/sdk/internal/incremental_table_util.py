...
def handle_resumed_run(incr_table: "Table", run: "LocalRun", key: str):
    """
    Since incremental tables create artifact entries using the increment number as
    a prefix, we need to make sure there are no naming collisions.
    ...
    if run.resumed and not incr_table._resume_handled:
        ...
        incr_table._previous_increments_paths = summary_from_key.get("previous_increments_paths", [])
...
def init_artifact(run: "LocalRun", sanitized_key: str):
    ...
    artifact = wandb.Artifact(artifact_name, "placeholder-run-incremental-table", incremental=True)
