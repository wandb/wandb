...
# ---------------------------------------------------------------------------
# Public entry points.  These free functions are the supported way to build a
# scheduler; callers should not instantiate `AxOptimizer` directly. Each returns
...
def resume_sweep(
    ...
    *,
    ...
    optimizer = AxOptimizer(options.client, resolved_sweep, options.terminator)
...
def create_sweep(
    ...
    *,
    ...
    """Create a sweep from the Ax experiment's space, then attach a scheduler.
    ...
    if options.client is None:
        raise ValueError("`options.client` is required")
    sweep = sweep_from_experiment(options.client, entity, project, program_path)
...
def create_sweep_from_config(
    ...
    *,
    ...
    optimizer = AxOptimizer(client, sweep, options.terminator)
    return InMemoryScheduler(
        optimizer,
        sweep,
        options,
