...
class Optimizer(ABC):
    ...
    def __init__(self, sweep: SweepInfo):
        self._sweep = sweep
        self._run_logger: ControllerRunLogger | None = None
        self.validate_sweep_objective()

    def attach_run_logger(self, run_logger: ControllerRunLogger) -> None:
        """Send this optimizer's `log()` lines to the sweep's logs.
    ...
    def log(self, message: str) -> None:
        ...
        self._run_logger.log(message, label=self.engine)
    ...
    def captured_loggers(self) -> Sequence[str]:
        ...
        output, and are appended to the sweep's user logs.
        """
        return ()
