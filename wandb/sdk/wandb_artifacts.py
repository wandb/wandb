...
from wandb.util import FilePathStr, StrPath, URIStr, to_posixpath
...
class Artifact(ArtifactInterface):
    ...
    def _add_local_file(
        self, name: StrPath, path: str, digest: Optional[B64MD5] = None
    ) -> ArtifactManifestEntry:
    ...
    def store_path(
        ...
        if blob_client.exists(version_id=query.get("versionId")):
            ...
            return [
                self._create_entry(
                    blob_properties,
                    path=name or to_posixpath(blob_name).name,
                    ref=URIStr(
        ...
        for i, blob_properties in enumerate(
            ...
            suffix = to_posixpath(blob_properties.name).relative_to(blob_name)
            entries.append(
                self._create_entry(
                    blob_properties,
                    path=to_posixpath(name) / suffix if name else suffix,
                    ref=URIStr(
                        f"{account_url}/{container_name}/{blob_properties.name}"
                    ),
                )
