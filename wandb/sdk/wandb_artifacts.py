...
class TrackingHandler(StorageHandler):
    ...
    def load_path(
        ...
        if local:
            ...
            url = urlparse(manifest_entry.ref)
            raise ValueError(
                f"Cannot download file at path {manifest_entry.ref!s}, "
                f"scheme {url.scheme} not recognized"
            )
...
class LocalFileHandler(StorageHandler):
    ...
    def load_path(
        ...
        if md5 != manifest_entry.digest:
            raise ValueError(
                f"Local file reference: Digest mismatch for path {local_path}: "
                f"expected {manifest_entry.digest} but found {md5}"
            )
        ...
        if version is None:
            ...
            if etag != manifest_entry.digest:
                if self.versioning_enabled(bucket):
                    ...
                    if obj is None:
                        raise ValueError(
                            f"Couldn't find object version for {bucket}/{key} "
                            f"matching etag {manifest_entry.extra.get('etag')}"
                        )
                else:
                    raise ValueError(
...
class GCSHandler(StorageHandler):
    ...
    def load_path(
        ...
        if obj is None:
            ...
            if obj is None:
                raise ValueError(
                    f"Unable to download object {manifest_entry.ref} version {version}"
                )
            md5 = obj.md5_hash
            if md5 != manifest_entry.digest:
                raise ValueError(
...
class HTTPHandler(StorageHandler):
    ...
    def load_path(
        ...
        if manifest_entry.digest != digest:
            raise ValueError(
                f"Digest mismatch for url {manifest_entry.ref}: "
                f"expected {manifest_entry.digest} but found {digest}"
            )
