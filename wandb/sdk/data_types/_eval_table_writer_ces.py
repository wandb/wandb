...
class _CESFieldType:
    ...
    extension_type: _eval_table_media_ces._CESExtensionType | None = None
    ...
    def write(self, payload: EvalTableWriteInput) -> EvalTableWriteResult:
        ...
        if write_payloads.oversized_media_cells:
            ...
            wandb.termwarn(
                ...
                f"{_eval_table_media_ces.CES_MAX_CELL_BYTES / 1_000_000:g} MB limit. "
    ...
    def _build_write_payloads(self, value: EvalTableWriteInput) -> _CESWritePayloads:
        ...
        for row_index, row in enumerate(value.rows):
            ...
            if row.outputs is not None:
                outputs, output_media, output_oversized = (
                    self._normalize_row_input_or_output_values(
                        row.outputs,
                        source="output",
                        column_keys=value.column_keys,
        ...
        dataset_fields: list[_CESDatasetField] = [
            dataset_field_types[(source, name)].declaration(source, name)
            for source, name in dataset_field_order
        ]
        scorers: list[_CESScorer] = [
