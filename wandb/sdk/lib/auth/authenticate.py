...
from .auth import Auth, AuthApiKey, AuthWithSource
...
def _try_env_auth(*, host: str) -> AuthWithSource | None:
    """Returns credentials from the WANDB_API_KEY env variable, if set."""
    if not (api_key := os.getenv(env.API_KEY)):
        return None
    ...
    return AuthWithSource(auth=auth, source=env.API_KEY)
