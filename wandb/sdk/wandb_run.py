...
_MAX_FOOTER_HISTORY_ROWS = 10
_MAX_FOOTER_SUMMARY_ROWS = 10
...
class Run:
    ...
    def _footer_history_summary_info(
        ...
        *,
        ...
        if history and (history_grid := Run._footer_history(history, printer)):
            panel.append(history_grid)

        if summary and (summary_grid := Run._footer_summary(summary, printer)):
    ...
    def _footer_history(
        ...
        for item in sorted_history_items:
            if len(history_rows) >= _MAX_FOOTER_HISTORY_ROWS:
                break
        ...
        if len(history_rows) < len(sorted_history_items):
            ...
            history_rows.append([f"(+ {remaining})", "..."])
    ...
    def _footer_summary(
        ...
        for item in sorted_summary_items:
            if len(summary_rows) >= _MAX_FOOTER_SUMMARY_ROWS:
                break
        ...
        if len(summary_rows) < len(sorted_summary_items) - skipped:
            ...
            summary_rows.append([f"(+ {remaining})", "..."])
