    ...
    def log(
        ...
        Args:
            ...
            commit: If true, finalize and upload the step. If false, then
                accumulate data for the step. When `commit=False`, data for
                different metrics is accumulated across multiple log calls,
                but if the same metric is logged multiple times with
                `commit=False`, only the last value will be written to history
                (previous values for that metric will be overwritten).
