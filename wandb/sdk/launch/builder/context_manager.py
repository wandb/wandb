...
class BuildContextManager:
    ...
    def generate_dockerfile(self, builder_type: str) -> str:
        ...
        dockerfile = launch_project.job_dockerfile or launch_project.override_dockerfile
        if launch_project.build_context:
            dockerfile = os.path.join(launch_project.build_context, dockerfile)
    ...
    def create_build_context(self, builder_type: str) -> Tuple[str, str]:
        ...
        context = self.launch_project.project_dir
        if self.launch_project.build_context:
            context = os.path.join(context, self.launch_project.build_context)
        if not os.path.isdir(context):
            raise LaunchError(
                f"Build context {self.launch_project.build_context} does not "
                "exist within job source code."
            )
        ...
        if entrypoint_dir:
            ...
            path = os.path.join(context, _WANDB_DOCKERFILE_NAME)
        if os.path.exists(
            path
        ...
        # This is the case where a Dockerfile path is specified in the job.
        if self.launch_project.job_dockerfile:
            dockerfile_abspath = os.path.join(
                context, self.launch_project.job_dockerfile
            )
            if not os.path.exists(dockerfile_abspath):
                raise LaunchError(
                    f"Dockerfile {self.launch_project.job_dockerfile} does not "
                    f"exist in {context}"
                )
            shutil.copytree(
                context,
            ...
            # Copy the Dockerfile to Dockerfile.wandb in build context root.
            shutil.copy(
                dockerfile_abspath, os.path.join(self.directory, _WANDB_DOCKERFILE_NAME)
            )
