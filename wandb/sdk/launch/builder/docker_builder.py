...
from .templates._wandb_bootstrap import FAILED_PACKAGES_PREFIX, FAILED_PACKAGES_POSTFIX
...
class DockerBuilder(AbstractBuilder):
    ...
    def build_image(
        ...
        try:
            _, output = docker.build(
                tags=[image_uri], file=dockerfile, context_path=build_ctx_path
            )
            _, stderr = output
            match = FAILED_PACKAGES_REGEX.search(stderr)
            if match:
                failed_packages = [p.strip() for p in match.group(1).split(",")]
                wandb.termwarn(
                    f"Failed to install the following packages: {failed_packages}. Will attempt to continue without them."
                )
