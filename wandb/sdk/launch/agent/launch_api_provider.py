...
class LaunchApiProvider:
    ...
    async def _get_job_api(self, job_tracker: JobAndRunStatusTracker) -> Optional[Api]:
        ...
        try:
            ...
            _logger.warning(
                f"Failed to get job API for {job_tracker.run_queue_item_id}: {e}"
            )
            return None

    async def _should_use_job_credentials(
        self, job_tracker: JobAndRunStatusTracker
    ) -> bool:
    ...
    async def remove_job_api_from_cache(
        self, job_tracker: JobAndRunStatusTracker
    ) -> None:
