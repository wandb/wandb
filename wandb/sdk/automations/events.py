...
from .filters import And, Gt, Gte, Lt, Lte
from .filters._expressions import AnyExpr, FilterExpr
from .scopes import ArtifactCollectionScope, ProjectScope, ScopeType

if TYPE_CHECKING:
    from .automations import ActionInput, PartialNewAutomation
...
class _OnEvent(Base):
    ...
    def add_action(self, action: ActionInput) -> PartialNewAutomation:
        """Define an executed action to be triggered by this event."""
        from .automations import PartialNewAutomation

        if isinstance(action, (DoNotification, DoWebhook)):
            return PartialNewAutomation(scope=self.scope, event=self, action=action)

        raise TypeError(f"Expected a valid action, got: {type(action).__qualname__!r}")

    def __rshift__(self, other: ActionInput) -> PartialNewAutomation:
...
class OnAddArtifactAlias(_OnEvent):
    ...
    @classmethod
    def from_pattern(
        cls, alias: str, *, scope: ArtifactCollectionScope | ProjectScope
    ) -> Self:
        return cls(
            scope=scope,
            filter=FilterExpr.model_validate({"alias": {"$regex": alias}}),
        )
...
class _QueryableField:
    ...
    def contains(self, text: str) -> RunFilter:
        field = self._alias or self._name
        filter_expr = FilterExpr.model_validate({field: {"$contains": text}})
...
class ArtifactEvent:
    @staticmethod
    def created(scope: ArtifactCollectionScope) -> OnCreateArtifact:
