...
from pydantic import AfterValidator, ConfigDict, Field, PositiveInt, field_validator
from typing_extensions import Annotated, Self, override

from wandb._pydantic import Base, GQLBase, SerializedToJson
...
class _WrappedEventFilter(GQLBase):
    filter: SerializedToJson[AnyOpDict | FilterExpr] = Field(default_factory=And)
...
class MetricFilter(GQLBase):
    ...
    @field_validator("agg_op", mode="before")
    @classmethod
    def _normalize_agg_op(cls, v: Any) -> Any:
        # Be helpful: e.g. "min" -> "MIN"
        return uppercase_if_str(v)

    @field_validator("cmp_op", mode="before")
    @classmethod
    def _normalize_cmp_op(cls, v: Any) -> Any:
        # Be helpful: e.g. ">" -> "$gt"
        return python_op_to_mongo(v)
...
_DEFAULT_INPUT_EVENT_FILTER = Or(inner=[And()]).model_dump_json()
...
class OnLinkArtifact(_EventInput):
    ...
    scope: _AnyScope
    filter: SerializedToJson[InputEventFilter] = _DEFAULT_INPUT_EVENT_FILTER

    @field_validator("filter", mode="after")
    @classmethod
    def _wrap_filter(cls, v: Any) -> Any:
        return _wrap_or_and(v)
