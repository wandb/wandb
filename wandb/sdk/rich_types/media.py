...
class MediaManager:
    ...
    def run_to_json(self) -> Dict[str, Any]:
        ...
        if self.artifact_path and self.artifact_path_latest:
            serialized.update(
                {
    ...
    def assign_artifact(self, artifact: "Artifact", path: str) -> Any:
        if self._artifact is not None or self._artifact_path is not None:
            raise ValueError(
                f"Media object already has an artifact ({self._artifact}/{self._artifact_path}) set."
            )
    ...
    def artifact_path(self) -> Optional[str]:
        if self._artifact is None or self._artifact_path is None:
            return None
        if self._artifact._client_id is None or not self._artifact._final:
            return None
        return f"wandb-client-artifact://{self._artifact._client_id}/{str(self._artifact_path)}"
    ...
    def artifact_path_latest(self) -> Optional[str]:
        ...
        if self._artifact._client_id is None or not self._artifact._final:
            return None
        return f"wandb-client-artifact://{self._artifact._sequence_client_id}:latest/{str(self._artifact_path)}"
    ...
    def bind_to_run(self, run, *namespace, suffix) -> None:
        ...
        dest_path = root_dir / self._relative_path / file_name
...
class Media:
    ...
    def __init__(self) -> None:
        self._manager = MediaManager(self.RELATIVE_PATH)

    @property
    def manager(self) -> MediaManager:
    ...
    def bind_to_artifact(self, artifact: "Artifact") -> Dict[str, Any]:
        ...
        """
        self._manager.bind_to_artifact(artifact)
        return {
            "_type": self.OBJ_ARTIFACT_TYPE,
            **self._manager.artifact_to_json(),
        }
    ...
    def bind_to_run(
        ...
        """
        assert self._manager._sha256 is not None
        name = name or self._manager._sha256[:20]
        self._manager.bind_to_run(run, *namespace, name, suffix=suffix)
