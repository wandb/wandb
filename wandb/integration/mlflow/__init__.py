...
def patch(settings=None, sync="all"):
    ...
    def _get_or_start_wandb_run(run, run_id=None, name=None):
        try:
            ...
            if run.data.tags.get("mlflow.parentRunId"):
                parent = RUNS.get(run.data.tags["mlflow.parentRunId"], {"run": None})[
                    "run"
                ]
            ...
            project = os.getenv(
                env.PROJECT, client.get_experiment(run.info.experiment_id).name
            )
    ...
    def _fix_step(run_id, metrics, step, timestamp):
        ...
        if (
            step in (None, 0)
            and int(time.time() - run_log["last_log"]) > LOG_FLUSH_MINIMUM
        ):
            wandb.termwarn(
