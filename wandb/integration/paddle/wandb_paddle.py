...
def nested_shape(array_or_tuple, seen=None):
    ...
    if hasattr(array_or_tuple, "size"):
        # paddle tensors use V.size() to get size of tensor
...
class PaddleHistory:
    """History methods specific to Paddle."""

    def __init__(self):
        global paddle
        paddle = wandb.util.get_module("paddle", "Could not import paddle")
        self._hook_handles = {}
        self._num_bins = 64
        self._is_cuda_histc_supported = None
        self.hook_paddle = PaddleGraph.hook_paddle
    ...
    def add_log_parameters_hook(
        ...
        """This instruments hooks into the paddle module.
        ...
        def parameter_log_hook(module: "Layer", input_, output, log_track):
    ...
    def add_log_gradients_hook(
        ...
        """This instruments hooks into the Paddle module slog gradients after a backward pass.

        Args:
            module: paddle.nn.Layer - the module to instrument
    ...
    def log_tensor_stats(self, tensor: "Tensor", name):  # noqa: C901
        ...
        if isinstance(tensor, (tuple, list)):
            ...
            tensor = paddle.concat([t.detach().clone().reshape([-1]) for t in tensor])
        ...
        if tensor.is_sparse():
            ...
            def numel(tensor_size):
                num = 1
                for s in tensor_size:
                    num *= s
                return num

            backing_values = tensor.values()
            sparse_zeros = numel(tensor) - backing_values.numel()
            tensor = backing_values

        flat: Tensor = tensor.reshape([-1])

        if flat.is_cuda:
            if self._is_cuda_histc_supported is None:
                try:
                    flat.histogram(bins=self._num_bins)
                except RuntimeError:
                    self._is_cuda_histc_supported = False
                else:
                    self._is_cuda_histc_supported = True

            # As of paddle 1.0.1.post2+nightly, float16 cuda summary ops are not supported (convert to float32)
            if not self._is_cuda_histc_supported:
                flat = flat.cpu()
            elif flat.dtype not in [paddle.float32, paddle.float64]:
                flat = flat.astype(paddle.float32)

        # Since we use histogram, we need to make sure that paddle supports the operation on CPU,
        # otherwise we'll get a runtime error. Hence, we need to upcast to float32.
        if not flat.is_cuda and flat.dtype not in [paddle.float32, paddle.float64]:
            flat = flat.astype(paddle.float32)
        ...
        if tmin > tmax:
            tmin, tmax = tmax, tmin
        if tmin == tmax:
            tensor = paddle.tensor([flat.numel().item()])
            tensor = tensor.cpu().clone().detach()
            bins = paddle.tensor([tmin, tmax])
        else:
            tensor = flat.histogram(bins=self._num_bins, min=tmin, max=tmax)
            tensor = tensor.cpu().detach().clone()
            bins = paddle.linspace(tmin, tmax, steps=self._num_bins + 1)
        ...
        if sparse_zeros:
            ...
            tensor = paddle.tensor(tensor_np)
            bins = paddle.tensor(bins_np)
    ...
    def _hook_variable_gradient_stats(self, var, name, log_track):
        """Logs a Tensor's gradient's distribution statistics next time backward() is called on it."""
        if not isinstance(var, paddle.Tensor):
            cls = type(var)
            raise TypeError(
                f"Expected paddle.Tensor, not {cls.__module__}.{cls.__name__}"
            )

        handle = self._hook_handles.get(name)
        if handle is not None and self._paddle_hook_handle_is_valid(handle):
    ...
    def _paddle_hook_handle_is_valid(self, handle):
    ...
    def _no_finite_values(self, tensor: "Tensor") -> bool:
        return (
            tensor.shape == paddle.Size([0]) or (~paddle.isfinite(tensor)).all().item()
        )

    def _remove_infs_nans(self, tensor: "Tensor") -> "Tensor":
        if not paddle.isfinite(tensor).all():
            tensor = tensor[paddle.isfinite(tensor)]

        return tensor


class PaddleGraph(wandb.data_types.Graph):
    def __init__(self):
        super().__init__("paddle")
        self._graph_hooks = set()

    @classmethod
    def hook_paddle(cls, model, criterion=None, graph_idx=0):
        wandb.termlog("logging graph, to disable use `wandb.watch(log_graph=False)`")
        graph = PaddleGraph()
        graph.hook_paddle_modules(model, criterion, graph_idx=graph_idx)
        return graph
    ...
    def hook_paddle_modules(
        self, module, criterion=None, prefix=None, graph_idx=0, parent=None
    ):
        paddle = util.get_module("paddle", "Could not import paddle")
        layers = 0
        graph = self
        ...
        for name, sub_module in module.named_children():
            ...
            if prefix:
                name = prefix + "." + name
            layers += 1
            if not isinstance(sub_module, paddle.nn.Layer):
                # TODO: Why does this happen?
                break

            module_types = [
                getattr(paddle.nn, module_classname)
                for module_classname in (
                    "Sequential",
                    "LayerList",
                    "LayerDict",
                )
                if hasattr(paddle.nn, module_classname)
            ]
            if parent is None:
                parent = module

            if isinstance(sub_module, tuple(module_types)):
                self.hook_paddle_modules(sub_module, prefix=name, parent=parent)
            else:
                ...
                try:
                    ...
                    wandb.run._paddle._hook_handles[
    ...
    @classmethod
    def from_paddle_layers(cls, module_graph, variable):
        """Recover something like neural net layers from Paddle Layer's and the compute graph from a Tensor.
        ...
        module_parameter_nodes = [
            n for n in module_graph.nodes if isinstance(n.obj, paddle.nn.Parameter)
        ]
        ...
        for pid, reachable_nodes in reachable_param_nodes.items():
            ...
            if not isinstance(node.obj, paddle.nn.Layer):
                continue
            module = node.obj
            ...
            for reachable_hash in reachable_nodes:
                ...
                if isinstance(reachable.obj, paddle.nn.Parameter):
                    param = reachable.obj
        ...
        compute_graph, compute_node_vars = cls.from_paddle_compute_graph(variable)
        for node, _ in reversed(list(compute_graph[0].ancestor_bfs())):
            ...
            if pid not in module_nodes_by_pid:
                # not all Parameters that occur in the compute graph come from the Layer graph
                continue
    ...
    @classmethod
    def node_from_module(cls, nid, module: "Layer"):
        ...
        for parameter in module.parameters():
            node.child_parameters += numpy.prod(parameter.size(), dtype="int64")
