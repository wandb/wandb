...
class DiffusersPipelineResolver:
    ...
    def __init__(self) -> None:
        ...
        self.supported_pipelines = SUPPORTED_MULTIMODAL_PIPELINES
    ...
    def __call__(
        ...
        """
        self.pipeline_name = args[0].__class__.__name__
        columns = []
        if self.pipeline_name in self.supported_pipelines:
            columns += self.supported_pipelines[self.pipeline_name]["table-schema"]
        else:
            wandb.Error("Pipeline not supported for logging")
        self.wandb_table = wandb.Table(columns=columns)
        loggable_dict = self.get_loggable_dict(args, kwargs, response)
        self.pipeline_call_count += 1
        return loggable_dict

    def get_loggable_dict(
        self,
        args: Sequence[Any],
        kwargs: Dict[str, Any],
        response: Response,
    ...
    def get_output_images(self, response: Response) -> List:
        ...
        """
        if "output-type" not in self.supported_pipelines[self.pipeline_name]:
            return response.images
        else:
            if self.supported_pipelines[self.pipeline_name]["output-type"] == "video":
                if self.pipeline_name in ["ShapEPipeline"]:
                    return response.images
                return response.frames
            elif self.supported_pipelines[self.pipeline_name]["output-type"] == "audio":
        ...
        if "output-type" not in SUPPORTED_MULTIMODAL_PIPELINES[self.pipeline_name]:
            try:
                ...
                if self.pipeline_name in [
                    ...
                    prompt_index = self.supported_pipelines[self.pipeline_name][
                        "kwarg-logging"
                    ].index("prompt")
                    prompt2_index = self.supported_pipelines[self.pipeline_name][
                        "kwarg-logging"
                    ].index("prompt_2")
    ...
    def add_data_to_table(
        ...
        """
        table_row = []
        kwarg_actions = self.supported_pipelines[self.pipeline_name]["kwarg-actions"]
    ...
    def prepare_loggable_dict(
        ...
        if self.pipeline_name in ["TextToVideoSDPipeline", "TextToVideoZeroPipeline"]:
            ...
            loggable_kwarg_ids = self.supported_pipelines[self.pipeline_name][
                "kwarg-logging"
            ]
            table_row = [
