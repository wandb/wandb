...
class File(Attrs):
    ...
    def __init__(
        self, client: RetryingClient, attrs: dict[str, Any], run: Run | None = None
    ):
        self.client = client
        self._attrs = attrs
        self.run = run
    ...
    def __repr__(self) -> str:
        return f"<{nameof(type(self))} {self.name} ({self.mimetype}) {to_human_size(self.size, units=POW_2_BYTES)}>"
