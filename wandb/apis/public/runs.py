...
class Run(Attrs):
    ...
    def purge_run_history_cache(self):
        """Purges the run history cache for the run."""
        cache_dir = env.get_cache_dir()
        run_history_cache_dir = cache_dir / "runhistory"

        glob_pattern = f"{self.entity}_{self.project}_{self.id}*.runhistory.parquet"
        files = glob.glob(os.path.join(run_history_cache_dir, glob_pattern))
        for file in files:
            wandb.termlog(f"Purging run history cache file: {file}")
            os.remove(file)
