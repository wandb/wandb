...
class Api:
    ...
    def automations(
        ...
        *,
        ...
        supports_no_op = self._supports_automation_action(ActionType.NO_OP.value)

        # For now, we need to use different queries depending on whether entity is given
        if entity is None:
            paginator = AutomationsForViewer(
                client=self.client,
                variables={"supportsNoOp": supports_no_op},
                per_page=per_page,
            )
        else:
            paginator = AutomationsByEntity(
                client=self.client,
                variables={"entityName": entity, "supportsNoOp": supports_no_op},
                per_page=per_page,
            )

        yield from filter(_should_keep, paginator)

    def _automation_event_types(self) -> Set[str]:
        """Return the set of automation event types that the server supports."""
        internal_api = InternalApi()
        allowed = internal_api.automation_event_and_action_types_introspection()
        return set(allowed.event_types)

    def _automation_action_types(self) -> Set[str]:
        """Return the set of automation action types that the server supports."""
        internal_api = InternalApi()
        allowed = internal_api.automation_event_and_action_types_introspection()
    ...
    def _check_server_support_for_automation(
        ...
        event_str = obj.triggering_event_type.value
        action_str = obj.triggered_action_type.value

        if event_str not in self._automation_event_types():
            raise ValueError(
                f"Server doesn't support automation with event: {event_str!r}"
            )
        if action_str not in self._automation_action_types():
            raise ValueError(
                f"Server doesn't support automation with action: {action_str!r}"
            )
