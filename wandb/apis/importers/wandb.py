...
class WandbRun:
    ...
    def artifacts(self) -> Optional[Iterable[Artifact]]:
        ...
        # self._artifacts = [self._download_and_process_artifact(art) for art in self._artifacts]
        # yield from self._artifacts
    ...
    #     for row in self.run.scan_history():
    #         row = remove_keys_with_none_values(row)
    #         row = self._modify_table_artifact_paths(row)
    #         yield row
    ...
    #     except Exception as e:
    #         logger.error(f"Error downloading file {f=}, {e=}")
    #     else:
...
class WandbImporter:
    ...
    def _import_run(
        ...
        *,
        ...
        # with self.progress_handler.task(f"Upload history: {run_str}", total=None):
    ...
    def _filter_for_failed_runs_only(self, runs):
        logger.debug("Filtering for failed runs only")
    ...
    def _cleanup_placeholder_runs(
        ...
        *,
        ...
        logger.info(f"Cleaning up placeholders, {namespaces=}")
    ...
    def import_runs(
        ...
        *,
        ...
        self._validate_runs(runs, skip_previously_validated=incremental)
        incremental_runs = list(self._filter_for_failed_runs_only(runs))
        ...
        logger.debug(f"Importing {len(incremental_runs)=} with {config=}")
        for_each(_import_run_wrapped, runs, max_workers=max_workers, parallel=parallel)
        logger.info("END: Import runs")
    ...
    def import_artifact_sequences(
        ...
        *,
        ...
        # destination as well
        self._cleanup_placeholder_runs(namespaces=namespaces)
        logger.info(f"END: Import artifact sequences, {namespaces=}")
    ...
    def _validate_runs(
        ...
        *,
        ...
        logger.debug(f"Validating {len(runs)=}, {skip_previously_validated=}")
...
@contextmanager
def test_context(key, url):
    old_key = os.environ["WANDB_API_KEY"]
    old_url = os.environ["WANDB_BASE_URL"]

    os.environ["WANDB_API_KEY"] = key
    os.environ["WANDB_BASE_URL"] = url
    yield
    os.environ["WANDB_API_KEY"] = old_key
    os.environ["WANDB_BASE_URL"] = old_url


def _get_run_from_art(art: Artifact, api=None):
    run = None
