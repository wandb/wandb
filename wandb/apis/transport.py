...
from .session import WandbSession
from .credentials import WandbCredentials
...
class RequestsHTTPTransport(HTTPTransport):
    def __init__(self, url, auth=None, use_json=False, timeout=None, **kwargs):
        ...
        self.credentials = WandbCredentials(url)
        self.session = WandbSession(self.credentials)
        self.auth = auth
