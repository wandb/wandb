...
@cli.command(
    ...
    help="""Verifies and stores your API key for authentication with W&B services.

             By default, only store credentials locally without verifying them with the W&B.
...
@cli.command(
    context_settings=CONTEXT,
    help="""Synchronize offline W&B run data to the cloud.

    By default, scan the current directory for a `wandb`
    subdirectory containing offline runs.

    If a path is provided, sync the runs found at the given paths.
    """,
)
@click.pass_context
...
@cli.command(
    "online",
    help="Enable live syncing with W&B when a network connection is available.",
)
@display_error
def online():
    api = InternalApi()
...
@cli.command(
    "offline",
    help="""Write metadata generated from W&B runs locally and does not sync to the cloud. 

             Make sure that the folder generated by W&B that contains your run's
             metadata is not deleted if you want to sync the run later.
             Use `wandb online` to re-enable syncing.
             """,
)
@display_error
def offline():
