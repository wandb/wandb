...
def _env_as_bool(
    var: str, default: Optional[str] = None, env: Optional[Mapping] = None
) -> bool:
    if env is None:
        env = os.environ
...
def is_debug(default: Optional[str] = None, env: Optional[Mapping] = None) -> bool:
...
def get_run(
    default: Optional[str] = None, env: Optional[Mapping] = None
) -> Optional[str]:
    if env is None:
        env = os.environ
...
def get_args(
    default: Optional[List[str]] = None, env: Optional[Mapping] = None
) -> Optional[List[str]]:
    if env is None:
        env = os.environ
    if env.get(ARGS):
...
def get_http_timeout(default: int = 10, env: Optional[Mapping] = None) -> int:
    if env is None:
        env = os.environ
...
def get_show_run(default: Optional[str] = None, env: Optional[Mapping] = None) -> bool:
    if env is None:
        env = os.environ
...
def get_tags(default: str = "", env: Optional[Mapping] = None) -> List[str]:
    if env is None:
        env = os.environ
...
def get_use_v1_artifacts(env: Optional[Mapping] = None) -> bool:
    if env is None:
        env = os.environ
...
def get_agent_max_initial_failures(
    default: Optional[int] = None, env: Optional[Mapping] = None
) -> Optional[int]:
    if env is None:
        env = os.environ
...
def set_entity(value: str, env: Optional[MutableMapping] = None) -> None:
    if env is None:
        env = os.environ
    env[ENTITY] = value


def set_project(value: str, env: Optional[MutableMapping] = None) -> None:
    if env is None:
        env = os.environ
...
def disable_git(env: Optional[Mapping] = None) -> bool:
    if env is None:
        env = os.environ
